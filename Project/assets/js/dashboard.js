document.addEventListener("DOMContentLoaded", function () {
    ...
    const statusBadges = document.querySelectorAll(".clickable-badge");
    ...
    statusBadges.forEach(function (badge) {
        ...
        badge.addEventListener("click", function () {
            ...
            let currentIndex = statuses.findIndex(function (status) {
            ...
            statuses.forEach(function (status) {
                badge.classList.remove(status.className);
            });

            badge.classList.add(nextStatus.className);
            badge.textContent = "• " + nextStatus.text;

            row.setAttribute("data-status", nextStatus.name);
    ...
    const editScheduleCard = document.getElementById("editScheduleCard");
    ...
    editButtons.forEach(function (button) {
        ...
        button.addEventListener("click", function () {
            ...
            const flightNumberElement = row.querySelector(".flight-no-link");
            const airlineElement = row.querySelector(".airline-text");
            const route = row.querySelectorAll(".route-iata");
            const times = row.querySelectorAll(".schedule-time-box span");
            const aircraftElement = row.querySelector(".aircraft-text");
            const gateElement = row.querySelector(".gate-bold-cyan");
            ...
            const departure = route[0].textContent.trim();
            const destination = route[1].textContent.trim();
            ...
            const gate = gateElement.textContent.trim();

            const editFlightId = document.getElementById("edit_flight_id");
            const editFlightNumber = document.getElementById("edit_flight_number");
            const editAirline = document.getElementById("edit_airline");
            const editDeparture = document.getElementById("edit_departure");
            const editDestination = document.getElementById("edit_destination");
            const editDepartureTime = document.getElementById("edit_departure_time");
            const editArrivalTime = document.getElementById("edit_arrival_time");
            const editAircraft = document.getElementById("edit_aircraft");
            const editGateNumber = document.getElementById("edit_gate_number");

            if (editFlightId) {
                editFlightId.value = this.getAttribute("data-id");
            }

            if (editFlightNumber) {
                editFlightNumber.value = flightNumber;
            }

            if (editAirline) {
                editAirline.value = airline;
            }

            if (editDeparture) {
                editDeparture.value = departure;
            }
            ...
            if (editDepartureTime) {
                editDepartureTime.value = departureTime;
            }

            if (editArrivalTime) {
                editArrivalTime.value = arrivalTime;
            }

            if (editAircraft) {
                editAircraft.value = aircraft;
            }

            if (editGateNumber) {
                editGateNumber.value = gate === "TBD" ? "" : gate;
            }

            if (editScheduleCard) {

                editScheduleCard.style.display = "block";
    ...
    if (closeEditFormBtn) {
        ...
        closeEditFormBtn.addEventListener("click", function () {
            ...
            if (editScheduleCard) {
                editScheduleCard.style.display = "none";
            }
    ...
    window.openGateModal = function (
        ...
        const modalGateId = document.getElementById("modalGateId");
        ...
        if (!gateModal) {
            console.error("gateModal not found");
            return;
        }

        if (modalGateId) {
            modalGateId.value = gateId;
        }

        if (modalGateNumber) {
            modalGateNumber.textContent = gateNumber;
        }
    ...
    window.openAssignFlightModal = function (
        flightId,
        flightNumber
    ) {
        ...
        const assignFlightId =
            document.getElementById("assignFlightId");

        const assignFlightNumber =
            document.getElementById("assignFlightNumber");

        if (!assignFlightModal) {
            console.error("assignFlightModal not found");
            return;
        }

        if (assignFlightId) {
            assignFlightId.value = flightId;
        }

        if (assignFlightNumber) {
            assignFlightNumber.textContent = flightNumber;
        }

        assignFlightModal.classList.add("show");

    };

    window.closeAssignFlightModal = function () {

        const assignFlightModal =
            document.getElementById("assignFlightModal");

        if (assignFlightModal) {
            assignFlightModal.classList.remove("show");
        }
    ...
    const baggageSearch =
        document.getElementById("baggageSearch");

    const baggageRows =
        document.querySelectorAll(".baggage-row");
    ...
    window.openBaggageEditModal = function (
        bagId,
        status
    ) {
        ...
        if (!modal) {
            console.error("baggageEditModal not found");
            return;
        }
    ...
    const baggageStatusForm =
        document.getElementById("baggageStatusForm");
